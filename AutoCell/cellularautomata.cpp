...
CellularAutomata::CellularAutomata(const TransitionRule* rule , Etat& dep, unsigned int nbEtats,unsigned int nombreVoisins,Voisinage const* definitionVoisinage,unsigned int buffer):
    m_rule(rule), m_etats(nullptr), m_depart(&dep), m_buffer(buffer),m_rang(0),m_nbEtats(nbEtats),m_nombreVoisins(nombreVoisins), m_voisinageDefinition(definitionVoisinage) {
    m_etats = new Etat*[m_buffer];
    for (unsigned int i = 0; i < m_buffer; i++) m_etats[i] = nullptr;
    //m_voisinageDefinition->definirVoisinage(*m_depart,m_nombreVoisins);
    m_etats[0] = new Etat(*m_depart);
    m_voisinageDefinition->definirVoisinage(*m_etats[0],m_nombreVoisins);
}
...
void CellularAutomata::Build(unsigned int cellule) {
    ...
    if (m_etats[cellule] == nullptr)
    {
        ...
        m_voisinageDefinition->definirVoisinage(*m_etats[cellule],m_nombreVoisins); //le voisinage est lié à la grille donc quand on recrée une grille on doit recréer un voisinage pour cette grille donc c'est important de ne pas avoir un buffer trop grand pour pas recalculer le voisinage trop de fois + pour éviter le surplus mémoire
